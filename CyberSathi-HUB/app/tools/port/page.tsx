...
import { Radar, ArrowLeft, Activity, AlertTriangle, Server, Network, Terminal } from 'lucide-react';
import Link from 'next/link';
import { analyzePorts } from '@/src/actions/port';
...
export default function PortScanner() {
  ...
  const runPortScan = async () => {
    ...
    const response = await analyzePorts(targetInput.trim());
      ...
      <div className="relative z-10 max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-12">
          ...
          <div className="flex items-center gap-3 bg-black/60 px-4 py-2 rounded-xl border border-neutral-800 backdrop-blur-md">
            ...
            <span className="text-xs font-mono text-neutral-300 tracking-wider">Engine: Port 5003</span>
          </div>
        </div>
        ...
        <div className="mb-10 flex items-center gap-6">
          <div className="w-16 h-16 rounded-2xl bg-black border border-neutral-800 flex items-center justify-center shadow-lg">
            <Radar className="w-8 h-8 text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]" />
          </div>
          <div>
            <h1 className="text-4xl font-bold tracking-tight mb-2 drop-shadow-md">Network Port Scanner</h1>
            <p className="text-neutral-400 font-mono text-sm">Service Enumeration & Vulnerability Mapping Engine</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-neutral-900/60 backdrop-blur-xl border border-neutral-800/80 p-8 rounded-3xl shadow-2xl h-fit">
            <label className="block text-xs font-mono text-red-500 mb-4 uppercase tracking-wider">Target IP or Hostname</label>
            ...
            <button 
              onClick={runPortScan}
              disabled={isScanning || targetInput.length === 0}
            ...
            >
              {isScanning ? (
                <><Activity className="w-5 h-5 animate-spin text-red-500" /> PROBING TARGET...</>
              ) : (
                <><Radar className="w-5 h-5" /> INITIATE TCP/UDP SCAN</>
              )}
            </button>
          </div>
          ...
          <div className="bg-black/90 border border-neutral-800/80 rounded-3xl shadow-2xl overflow-hidden flex flex-col min-h-[400px]">
            <div className="bg-neutral-900/80 border-b border-neutral-800 px-6 py-3 flex items-center gap-4">
              ...
              <span className="text-xs font-mono text-neutral-400 tracking-widest uppercase">Target Enumeration</span>
            </div>
            ...
            <div className="p-6 font-mono text-sm overflow-y-auto flex-1 relative flex flex-col justify-center">
              {!isScanning && !report && !error && (
                <div className="text-neutral-600 text-center">
                  [ Enter target to begin port enumeration ]
                </div>
              )}

              {isScanning && (
                <div className="text-red-500 animate-pulse space-y-2">
                  <p>&gt; Establishing connection to Port 5003...</p>
                  <p>&gt; Resolving target hostname...</p>
                  <p>&gt; Sending TCP SYN packets...</p>
                  <p>&gt; Analyzing response headers & banners...</p>
                </div>
              )}
              ...
              {report && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 w-full">
                  <div className="text-emerald-500 mb-4 flex items-center gap-2">
                    <Network className="w-4 h-4" /> 
                    <span>&gt; Scan Complete. Open services mapped:</span>
                  </div>
