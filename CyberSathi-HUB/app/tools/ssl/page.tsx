...
import { Lock, ArrowLeft, Activity, AlertTriangle, ShieldCheck, ShieldAlert, Terminal, Globe } from 'lucide-react';
import Link from 'next/link';
import { analyzeSsl } from '@/src/actions/ssl';
...
export default function SslChecker() {
  ...
  const runSslScan = async () => {
    ...
    const response = await analyzeSsl(domainInput.trim());
      ...
      <div className="relative z-10 max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-12">
          ...
          <div className="flex items-center gap-3 bg-black/60 px-4 py-2 rounded-xl border border-neutral-800 backdrop-blur-md">
            ...
            <span className="text-xs font-mono text-neutral-300 tracking-wider">Engine: Port 5006</span>
          </div>
        </div>
        ...
        <div className="mb-10 flex items-center gap-6">
          ...
          <div>
            <h1 className="text-4xl font-bold tracking-tight mb-2 drop-shadow-md">SSL/TLS Cryptography Scanner</h1>
            <p className="text-neutral-400 font-mono text-sm">Certificate Validation & Handshake Analysis Engine</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-neutral-900/60 backdrop-blur-xl border border-neutral-800/80 p-8 rounded-3xl shadow-2xl h-fit">
            <label className="block text-xs font-mono text-red-500 mb-4 uppercase tracking-wider">Target Domain Name</label>
            ...
            <button 
              onClick={runSslScan}
              disabled={isScanning || domainInput.length === 0}
            ...
            >
              {isScanning ? (
                <><Activity className="w-5 h-5 animate-spin text-red-500" /> INTERROGATING SERVER...</>
              ) : (
                <><Lock className="w-5 h-5" /> INITIATE SSL HANDSHAKE</>
              )}
            </button>
          </div>
          ...
          <div className="bg-black/90 border border-neutral-800/80 rounded-3xl shadow-2xl overflow-hidden flex flex-col min-h-[400px]">
            <div className="bg-neutral-900/80 border-b border-neutral-800 px-6 py-3 flex items-center gap-4">
              ...
              <span className="text-xs font-mono text-neutral-400 tracking-widest uppercase">Certificate Handshake Telemetry</span>
            </div>
            ...
            <div className="p-6 font-mono text-sm overflow-y-auto flex-1 relative flex flex-col justify-center">
              {!isScanning && !report && !error && (
                <div className="text-neutral-600 text-center">
                  [ Enter target domain to pull cert data ]
                </div>
              )}

              {isScanning && (
                <div className="text-red-500 animate-pulse space-y-2">
                  <p>&gt; Establishing connection to Port 5006...</p>
                  <p>&gt; Initiating TLS ClientHello...</p>
                  <p>&gt; Extracting X.509 Certificate Chain...</p>
              ...
              {error && (
                <div className="text-red-500 bg-red-500/10 border border-red-500/20 p-4 rounded-lg flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-bold mb-1">HANDSHAKE FAILED</p>
              ...
              {report && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 w-full flex flex-col items-center">
                  
                  {/* Dynamic Status Icon */}
                  <div className="mb-6">
                    {report.error || String(report).includes("error") ? (
                  ...
                  <div className="text-emerald-500 mb-4 w-full text-left">&gt; Handshake Complete. X.509 Data Extracted:</div>
