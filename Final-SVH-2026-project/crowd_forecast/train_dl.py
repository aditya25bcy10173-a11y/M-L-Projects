...
# Define PyTorch Multi-Task Neural Network
...
def main():
    ...
    # Fit preprocessor on entire features set
    ...
    test_dataset = CrowdDataset(X_test, y_footfall_test, y_temp_test, y_rain_test)
    ...
    epochs = 80
    print(f"\nTraining PyTorch model for {epochs} epochs on CPU...")
    model.train()
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_x, batch_yf, batch_yt, batch_yr in train_loader:
            ...
            loss_f = criterion(pred_f, batch_yf)
            loss_t = criterion(pred_t, batch_yt)
            loss_r = criterion(pred_r, batch_yr)
            
            # Summed loss (since features are standardized, loss scales are uniform)
            loss = loss_f + loss_t + loss_r
    ...
    with torch.no_grad():
        ...
        metrics_path = os.path.join(output_dir, "dl_metrics.txt")
        with open(metrics_path, "w") as f:
            f.write("PyTorch Multi-Task Deep Learning Model Evaluation\n")
            f.write("===============================================\n\n")
        ...
        plt.tight_layout()
        plot_path = os.path.join(output_dir, "dl_predictions_comparison.png")
